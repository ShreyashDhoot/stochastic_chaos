...
def generate_greedy(model, tokenizer, prompt: str, max_new_tokens: int = 1024) -> Tuple[str, float]:
    ...
    """
    messages = [{"role": "system", "content": "reason out the logic in steps and give the final answer.Final answer should be number or true/false"},
...
def generate_multi_sample(model, tokenizer, prompt: str, k: int, 
    ...
    #build prompt
    messages = [
        {"role": "system", "content": "reason out the logic in steps and give the final answer.Final answer should be number or true/false"},
        {"role": "user", "content": prompt}
    ]
